...
public partial class InformeEfectividadMedidor : System.Web.UI.Page
{
        ...
        public void Carga_Improcedentes_Repaso()
    {
        ...
        int total_repaso = 0,primer_repaso=0,segundo_repaso=0;

        foreach (DataRow drOrdenItem in dtOrdenes.Rows)
        {

            string primer_rep = drOrdenItem["PRIMER_REPASO"].ToString();
            string segundo_rep = drOrdenItem["SEGUNDO_REPASO"].ToString();

            if (primer_rep == "" && segundo_rep == "")
            {
                primer_repaso = 0;
                segundo_repaso = 0;
                total_repaso = 0;
            ...
            else
            {
                ...
                primer_repaso = int.Parse(drOrdenItem["PRIMER_REPASO"].ToString());
                segundo_repaso = int.Parse(drOrdenItem["SEGUNDO_REPASO"].ToString());
                total_repaso = primer_repaso + segundo_repaso;
            }

        }

        TextBox3.Text = primer_repaso.ToString();
        TextBox1.Text = segundo_repaso.ToString();
        TextBox5.Text = total_repaso.ToString();
    ...
    public void Estadistica()
    {
        ...
        double porcentaje_rep1 = double.Parse(TextBox3.Text.ToString());
        double porcentaje_rep2 = double.Parse(TextBox1.Text.ToString());
        double total_repaso = double.Parse(TextBox5.Text.ToString());

        if (totalEfec != 0 || totalImp != 0)
        {
            cambioVisPor = totales * 100 / totalPorEfc;
            txt_leidas.Text = Math.Round(cambioVisPor, 2).ToString();
            cambioPorVis = toti * 100 / totalPorEfc;
            txt_pendientes.Text = Math.Round(cambioPorVis, 2).ToString();
            totalCambEf = totalEfec * 100 / totalPorEfc;
            txt_efectivos.Text = Math.Round(totalCambEf, 2).ToString();
            totalCambImp = (totalImp * 100) / totalPorEfc;
            txt_sin_lecturas.Text = Math.Round(totalCambImp, 2).ToString();
            TextBox4.Text = Math.Round((porcentaje_rep1 * 100) / totalImp, 2).ToString();
            TextBox2.Text = Math.Round((porcentaje_rep2 * 100) / totalImp, 2).ToString();
            TextBox6.Text = Math.Round((total_repaso * 100) / totalImp, 2).ToString();
