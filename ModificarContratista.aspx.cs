...
public partial class ModificarContratista : System.Web.UI.Page
{
    string id_perfil;
    ...
    public void check()
    {
        ...
        {
            CheckEstadoE.Text = "Inactivo";
            CheckEstadoE.ForeColor = System.Drawing.Color.Red;

        }
        else
        {
            CheckEstadoE.Text = "Activo";
    ...
    public void carga_contratista_porID() 
    {
        string ID_ContrartistaC = Request.QueryString["idContratista"]; 
        ...
        objConexion.Open();
        sSQL = "SELECT CONTRATISTA.ID_CONTRATISTA, CONTRATISTA.NOMBRE, CONTRATISTA.RUT, CONTRATISTA.COD_CONTRATISTA, CONTRATISTA.RUT_EMPRESA, " +
        "CONTRATISTA.ESTADO, CONTRATISTA.FECHA_CREACION FROM CONTRATISTA JOIN EMPRESA ON CONTRATISTA.RUT_EMPRESA =EMPRESA.RUT_EMPRESA WHERE CONTRATISTA.ID_CONTRATISTA =" + ID_ContrartistaC;

        objComando = new OracleDataAdapter(sSQL, conexion_cliente_oracle);
        objComando.Fill(dtContratista); 
        objConexion.Close();
        objConexion.Dispose();
        ...
        foreach (DataRow drContratista in dtContratista.Rows) 
        {
            ...
            dr["estado"] = drContratista["estado"].ToString();
        ...
        txtNombreContratistaE.Text = dt.Rows[0]["nombre"].ToString();
        txtRutE.Text = dt.Rows[0]["rut"].ToString();
        txtCodigoContratistaE.Text = dt.Rows[0]["cod_contratista"].ToString();
        txtFechaCreacionE.Text = dt.Rows[0]["fecha_creacion"].ToString();
        ...
        txtRutE.Text = arr_RUT[0];
        txt_dvE.Text = arr_RUT[1];



        if (dtContratista.Rows[0]["estado"].ToString() == "0")
        {
    ...
    protected void imgBtoFechaCreacion_Click(object sender, System.Web.UI.ImageClickEventArgs e)
    {

    }
    protected void btoEditarContratista_Click(object sender, EventArgs e)
    {
         string ID_CONTRATISTA = Request.QueryString["idContratista"];  
        ...
        //            VALIDACION_RUT_USUARIO.Visible = false;
                    ...
                    ComandoDA = new OracleDataAdapter("PK_CONTRATISTA.UPDATE_CONTRATISTA", objConexion);

                    ComandoDA.SelectCommand.CommandType = CommandType.StoredProcedure;

                        PRO.agrega_parametro_sp(ComandoDA, "i_id_contratista", OracleType.Int32, ParameterDirection.Input, ID_CONTRATISTA.ToString());
                        PRO.agrega_parametro_sp(ComandoDA, "i_nombre", OracleType.NVarChar, ParameterDirection.Input, txtNombreContratistaE.Text);
                        PRO.agrega_parametro_sp(ComandoDA, "i_rut", OracleType.NVarChar, ParameterDirection.Input, txtRutE.Text + "-" + txt_dvE.Text);
                        PRO.agrega_parametro_sp(ComandoDA, "i_cod_contratista", OracleType.NVarChar, ParameterDirection.Input, txtCodigoContratistaE.Text);
                        PRO.agrega_parametro_sp(ComandoDA, "i_fecha_creacion", OracleType.DateTime, ParameterDirection.Input, txtFechaCreacionE.Text);
